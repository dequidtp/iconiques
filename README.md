...
- **Throttling Google News** : le flux limite le débit depuis une IP partagée (runner GitHub Actions). Le scanner espace donc ses requêtes (`REQUEST_DELAY_MS`, 3 s) et réessaie avec backoff sur 429/5xx — un run complet prend ~6 min pour 40 entreprises. **Si la collecte échoue pour une entreprise, aucun snapshot n'est écrit** : son score précédent est conservé plutôt que remis à 0. Le récapitulatif de fin de run indique combien d'entreprises ont échoué ; si le chiffre est élevé, augmente `REQUEST_DELAY_MS`.
- **Noms de PDG à vérifier** : la détection repose sur `ceo_name`. Un nom périmé (les dirigeants changent souvent) = zéro résultat pour cette société. Si une entreprise reste durablement à 0 alors que son PDG s'exprime, c'est le premier champ à contrôler dans la table `companies`.
